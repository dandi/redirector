...
from dandi.download import follow_redirect, parse_dandi_url
from dandi.exceptions import NotFoundError

REDIRECTOR_URL_RGX = re.compile(r"^https?://dandiarchive\.org(/.*)?$")


def call_sanic(req, method):
    m = REDIRECTOR_URL_RGX.match(req.url)
    assert m
    path = m.group(1) or "/"
    _, r = getattr(app.test_client, method)(path)
...
def sanic_responses():
    with responses.RequestsMock(assert_all_requests_are_fired=False) as resps:
        resps.add_callback(
            ...
            callback=partial(call_sanic, method="get"),
        )
        resps.add_callback(
            responses.HEAD,
            re.compile(r"^https?://dandiarchive\.org"),
            callback=partial(call_sanic, method="head"),
        )
        resps.add_passthru(re.compile(r".*"))
        yield resps
...
def test_parse_dandi_url(sanic_responses):
...
def test_parse_dandi_url_redirect(sanic_responses):
